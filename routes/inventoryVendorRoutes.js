...
const { list, create, update, remove } = require("../models/inventoryVendorModel");
...
router.post("/", async (req, res) => {
  try {
    ...
    res.status(isDup ? 409 : 400).json({
      message: isDup ? "A vendor with this name already exists" : error.message || "Failed to create",
    });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const isDup = error?.code === "ER_DUP_ENTRY";
    res.status(isDup ? 409 : 400).json({
      message: isDup ? "A vendor with this name already exists" : error.message || "Failed to update",
    });
  }
});
